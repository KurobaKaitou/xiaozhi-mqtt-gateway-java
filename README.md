...
- `inbound-topic`：设备上行订阅主题（默认 `device/+/up`）
- `outbound-topic-template`：主下行主题模板（默认 `devices/p2p/{macRaw}`）
...
### 设备下行（subscribe，至少订阅一个）

- `devices/p2p/{macRaw}`（主通道）
- `null`（兼容）
- `device/{clientId}/down`（兼容）
